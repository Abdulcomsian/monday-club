...
@section('content')
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        <div class="container-fluid fluid">
            ...
            <div class="row">
                <div class="col-xl-12">
                    <form action="{{ route('user.donations.update', $data->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="row">
                                <div class="col-xl-6 d-flex flex-column">
                                    ...
                                    <select name="contact_id" id="contactSelect" class="form-select" required>
                                        ...
                                        @foreach ($data->contact as $value)
                                            <option value="{{ $value->id }}" {{ isset($data) && $data->contact_id == $value->id ? 'selected' : '' }}>
                                                {{ $value->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-xl-6 d-flex flex-column">
                                    ...
                                    @error('amount')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                    <input type="number" class="form-control" name="amount" value="{{ $data->amount }}" placeholder="amount here...">
                                </div>
